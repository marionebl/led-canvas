	...
	write(str, size=1, x = 0, y = 0) {
		...
		str.split('').forEach((char) => {
			...
			if (this.cursor.x + sign.width + charSpacing*2 > this.matrix.x) {
				this.cursor = { x: charSpacing, y: this.cursor.y + this.font.meta.lineHeight + charSpacing*2 };
			}
			...
			this.cursor.x += this.font.meta.monospaced ? this.font.meta.charWidth + charSpacing*2 : sign.width + charSpacing*2;
		});
	}
}
